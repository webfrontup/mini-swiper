;(function($, window, document, undefined) {
    var Carousel = function(elem, options) {
        ...
        this.$carousel.append('<div class="pk-football"></div>')
        $("#pk_carouseltop").append('<div id=bannerNav>' + htmlNav + '</div>');
        this.$aNav = $("#pk_carouseltop").children('#bannerNav').find('ul li');
    };

    var outerWidth = parseInt(document.body.offsetWidth);
    var middleWidth = 1920;
    var _height = outerWidth >= middleWidth ? 380 : 226; //中间的高度
    var _slideHeight = outerWidth >= middleWidth ? 330 : 177; //侧边的高度
            ...
            for (var i = 0; i < half_imgLen; i++) {
                ...
                if (k == 0) {
                    this.$aImg.eq(leftNum).css({
                        transform: 'translateX(' + (-264 * (i + 1)) + 'px) translateZ(-120px)',
                        width:"632px",
                    }).animate({
